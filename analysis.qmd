...
- a numeric input with the amount of *whales* (that is: the top N downloaders); stored in `input$n_whales`. 
...
- I created a module for each page and appended the app version to it (for example, `md1.all_traffic_UI` means "module for app 1"). I also prepared all data in server before passing these to the modules. It will be easier to optimize the app later in this setting, and I pass to each module just the essential, nothing more.
